...
dcomplex addSSS( dcomplex a, dcomplex b) { 
  ...
  sum.re = a.re + b.re;   // Real part addition
  sum.im = a.im + b.im;   // Imaginary part addition
   return sum;
} 
...
int addSPI(dcomplex* sum, dcomplex a, dcomplex b) {
   ...
   sum->re = a.re + b.re;  // Real part addition using pointer
   sum->im = a.im + b.im;  // Imaginary part addition using pointer
   return 0; 
} 
...
int addPPP(dcomplex* sum, dcomplex *a, dcomplex *b) {
   // Perform complex arithmetic using the pointer
   sum->re = a->re + b->re; // Real part addition using pointers
   sum->im = a->im + b->im; // Imaginary part addition using pointers
   return 0; 
}
