/*---------------------------------------------------------------------------
  ...
  gcc -g lab_b.c   DynamicArrays.c  -DHW11 -std=c99 -lgsl -lgslcblas -o lab_b
---------------------------------------------------------------------------*/
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
void prepareDataForLS(int nr, int nc, const DArray *points) {
   ...
   for(int i = 0; i < nr; i++) {
      ...
      // **update**
      double x_value = (points->Payload[i]).x; // Accessing x from DynamicArrays
      gsl_matrix_set(A, i, 0, 1.0); // The first column is all 1's for the intercept
      double current_power = x_value;
      for(int j = 1; j < nc; j++) {
         gsl_matrix_set(A, i, j, current_power); // Powers of x for polynomial terms
         current_power *= x_value; // Manually calculate the next power of x
      }
   } /* End for i */
   ...
   for(int i = 0; i < nr; i++) {
      // **update**
       gsl_vector_set(b, i, (points->Payload[i]).y); // Accessing y from DynamicArrays
   }
