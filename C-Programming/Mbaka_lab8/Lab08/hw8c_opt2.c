...
int main(int argc, char *argv[])
  ...
  BEGIN_REPEAT_TIMING(R1, repeatVar) 
   ...
   /* Insert end calc timing */
    ...
    /* Print out a message indicating the simple loop unrolling implementation. */
    printf("Adds native loop unrolling to the base code. \n");
...
void CalculateCoefficients(LinearFit *DataSet, double *A, double *B)
{
    ...
    /* Compute the sums with loop unrolling by two */
    for (lcv = 0; lcv <= DataSet->NextElement - 2; lcv += 2)
    {
        S_XX += DataSet->Data_X[lcv] * DataSet->Data_X[lcv] + DataSet->Data_X[lcv + 1] * DataSet->Data_X[lcv + 1];
        S_XY += DataSet->Data_X[lcv] * DataSet->Data_Y[lcv] + DataSet->Data_X[lcv + 1] * DataSet->Data_Y[lcv + 1];
        S_X += DataSet->Data_X[lcv] + DataSet->Data_X[lcv + 1];
        S_Y += DataSet->Data_Y[lcv] + DataSet->Data_Y[lcv + 1];
    }
