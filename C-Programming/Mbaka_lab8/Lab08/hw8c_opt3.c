 ...
 **  03/10/2021  R. Repka     Removed MOVE_IO_CLOSE
 **************************************************************************/
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
#ifndef R1
#define R1 100  // Define the number of repetitions
#endif
...
int main(int argc, char *argv[])
  {
    ...
    DECLARE_TIMER(DataTimer); // Declare a timer for data reading
    DECLARE_TIMER(CalcTimer); // Declare a timer for calculations
  ...
  BEGIN_REPEAT_TIMING(R1, repeatVar) 
   ...
   /* Insert end calc timing */
    ...
    /* Print out a message indicating the more complex loop unrolling implementation. */
    printf("Adds smarter loop unrolling to the base code. \n");
...
void CalculateCoefficients(LinearFit *DataSet, double *A, double *B)
{
    // Declare and initialize sum variables for each part of the calculation
    double S_XX1 = 0.0, S_XX2 = 0.0;
    double S_XY1 = 0.0, S_XY2 = 0.0;
    double S_X1  = 0.0, S_X2  = 0.0;
    double S_Y1  = 0.0, S_Y2  = 0.0;

    // Loop unrolling by a factor of two
    int lcv;
    for (lcv = 0; lcv <= DataSet->NextElement - 2; lcv += 2)
    {
        // First part of the unrolled loop
        S_XX1 += DataSet->Data_X[lcv] * DataSet->Data_X[lcv];
        S_XY1 += DataSet->Data_X[lcv] * DataSet->Data_Y[lcv];
        S_X1  += DataSet->Data_X[lcv];
        S_Y1  += DataSet->Data_Y[lcv];

        // Second part of the unrolled loop
    ...
    // Handle any remaining element if the dataset size is odd
    if (DataSet->NextElement % 2 != 0)
    {
        S_XX1 += DataSet->Data_X[lcv] * DataSet->Data_X[lcv];
        S_XY1 += DataSet->Data_X[lcv] * DataSet->Data_Y[lcv];
        S_X1  += DataSet->Data_X[lcv];
    ...
    // Sum up the variables from both parts of the unrolled loop
    double S_XX = S_XX1 + S_XX2;
    double S_XY = S_XY1 + S_XY2;
    double S_X  = S_X1  + S_X2;
    double S_Y  = S_Y1  + S_Y2;
