...
int main(int argc, char *argv[])
  ...
  BEGIN_REPEAT_TIMING(R1, repeatVar) 
   ...
   /* Insert end calc timing */
    ...
    /* Print out a message indicating the realloc() implementation. */
    printf("Adds x2 memory growth and realloc to the base code\n");
 ...
 // int tmp_old_size; /* temp variable to store size when full */
  ...
  double *NewData_Y = NULL;
   ...
   if (++DataSet->NextElement >= DataSet->Size)
     {
      /* Temp variable to store old size for comparison */
     // tmp_old_size = DataSet->Size;

      /* Double the size of the arrays if non-zero, otherwise set to 1 */
      DataSet->Size = DataSet->Size > 0 ? DataSet->Size * 2 : 1;

      /* Reallocate the arrays to the new (larger) size */
      NewData_X = (double *)realloc(DataSet->Data_X, sizeof(double) * DataSet->Size);
      NewData_Y = (double *)realloc(DataSet->Data_Y, sizeof(double) * DataSet->Size);
      
      /* Check for any errors */
      if ((NULL == NewData_X) || (NULL == NewData_Y)) 
         {
          fprintf(stderr, "Error: Could not allocate memory at line %d\n", __LINE__);
          // Free the original arrays only if realloc fails, to avoid memory leaks
          free(DataSet->Data_X);
          free(DataSet->Data_Y);
          exit(-99);
         }
